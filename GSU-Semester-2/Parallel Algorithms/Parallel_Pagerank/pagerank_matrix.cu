...
int vertexInArray(int *ar, int length, unsigned int vertex) {
  int present = 0;
  if (length != 0) {
...
/*************************************************************************/
  ...
  FILE * fp;
  if ((fp = fopen("/home/users/vbhamidipati1/project/web-dataset/web-Google/smaller-web-Google.txt", "r")) == NULL) {
  ...
  printf("***********PARSING INPUT FILE TO COUNT NUMBER OF VERTICES******************");
  while (fscanf(fp, "%u %u", &vertex_from, &vertex_to) != EOF && startLine <= endLine) {
    // if (vertex_from > n_vertices)
    //   n_vertices = vertex_from;
    // else if (vertex_to > n_vertices)
    //   n_vertices = vertex_to;
    if (!vertexInArray(arr, used, vertex_from)) {
      arr[used] = vertex_from;
      used++;
    }

    if (!vertexInArray(arr, used, vertex_to)) {
      arr[used] = vertex_to;
      used++;
    }   
    
    if (used == capacity) {
      capacity*=2; // float the capacity of array
  ...
  n_vertices = used + endLine / 5;
    ...
    ii =0;
    startLine = 1;
    printf("***************CALCULATING OUTGOING LINKS PER PAGE***********");
    while(fscanf(fp,"%u %u", &vertex_from, &vertex_to) != EOF && startLine <= endLine) {
        outgoingLinks[vertex_from] += 1;
        printf("Outgoing links: %u\n", outgoingLinks[vertex_from]);
        printf("%d\n", ii);
        ii++;
        startLine++;
    }
 ...
 // err = cudaMemcpy(d_vertices, vertices, sizeOfVertices, cudaMemcpyHostToDevice);
    printf("After Assigning memory for vertices");
    // CREATE MATRIX  **********************************************************
    float ** matrix;
   ...
   // matrix = (float **) malloc(n_vertices * sizeof(float*));
    ...
    for(i=0; i < n_vertices; i++) {
       // matrix[i] = (float *) malloc(n_vertices * sizeof(float));
        ...
        printf("Assigned Memory for vertex:%d\n", i + 1);
    ...
    // float initialPageranks = 0.1;
    float * pagerankMatrix;
    printf("*****************ASSIGNING MEMORY FOR PAGE RANK MATRIX**************");
    err = cudaMallocManaged((void**)&pagerankMatrix,n_vertices*sizeof(float));
    printf("*****************ASSIGNED MEMORY FOR PAGE RANK MATRIX**************");
    for(i=0;i<n_vertices;i++) {
    ...
    // float danglingCorrection = 1.0/1000;
    // float initialVal = (1.0-alpha)/1000;
    ...
    ii=0;
    startLine = 1;
    printf("****************FINISH MATRIX INITIAL VALUE SETUP***********************");
    while(fscanf(fp,"%u %u", &vertex_from, &vertex_to) != EOF && startLine <= endLine) {
...
/////////////////////END OF MY MATRIX TEST/////////////////////////////////////////////
 ...
 if (!vertices) {
  ...
  ii = 0;
  startLine = 1;
  printf("******************COUNTING SUCCESSORS OF EACH VERTEX**********************");
  while (fscanf(fp, "%u %u", &vertex_from, &vertex_to) != EOF && startLine <= endLine) {
  ...
  printf("**************Allocating memory for successor pointers***************\n");
  ...
  ii =0;
  startLine = 1;
  printf("******************PARSING INPUT FILE TO SET UP SUCCESSOR POINTERS******************");
  while (fscanf(fp, "%d %d", &vertex_from, &vertex_to) != EOF && startLine <= endLine) {
    for (i=0; i<vertices[vertex_from].n_successors; i++) {
      if (vertices[vertex_from].successors[i] == NULL) {
        ...
        printf("Setting up the successor pointers of vertex %u failed",vertex_from);
        return -1;
      }
    }
    startLine++;
